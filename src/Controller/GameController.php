...
class GameController extends AbstractController
{
    ...
    public function update(Game $game, Request $request, EntityManagerInterface $entityManager): JsonResponse
    {
        ...
        $newStats = $data['statistics'] ?? [];
        $game->setParametrs($newStats);

        $season = $entityManager->getRepository(Season::class)->findOneBy([
    ...
    private function countGoals(array $actions): array
    {
        $goals = [];
        foreach ($actions as $act) {
            if (isset($act['type'], $act['id']) && (int)$act['type'] === 1) {
                $pid = (int)$act['id'];
